...
def do_eval_step(batch, head_outputs, loss, eval_kwargs):
    ...
    n_batch_gt_rel, n_batch_pr_rel = eval_el_spade_batch(
        pr_el_labels,
        batch["el_labels"],
    ...
    step_out = {
        "loss": loss,
        "batch_gt_rel": n_batch_gt_rel,
        "batch_pr_rel": n_batch_pr_rel,
    }
...
def eval_el_spade_batch(
    ...
    batch_gt_rel, batch_pr_rel = [], []
    for example_idx in range(bsz):
        gt_rel, pr_rel = eval_el_spade_example(
        ...
        # print("======= pt", pr_rel)
        batch_gt_rel.append(gt_rel)
        batch_pr_rel.append(pr_rel)

    return batch_gt_rel, batch_pr_rel
...
def eval_el_spade_example(pr_el_label, gt_el_label, box_first_token_mask, dummy_idx):
    ...
    return gt_relations, pr_relations
